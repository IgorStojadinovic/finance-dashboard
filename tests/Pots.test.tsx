...
import Pots from '../src/components/pages/ports/Pots';
...
vi.mock('../src/components/modals/EditingModals', () => ({
  default: () => (
    <div data-testid='mocked-editing-modals'>Editing Modals Component</div>
  ),
}));
...
describe('Pots Page', () => {
  it('renders pots section with correct information', () => {
    ...
    const savingName = within(savingPotItem).getByRole('heading', {
      name: /saving/i,
    });
    ...
    const concertName = within(concertPotItem).getByRole('heading', {
      name: /concert ticket/i,
    });
  ...
  it('renders editing modals components for each pot', () => {
    ...
    const editingModalsComponents = screen.getAllByTestId(
      'mocked-editing-modals'
    );
  ...
  it('displays correct progress bar information', () => {
    ...
    const firstPotProgressBar = screen.getByTestId(
      `pot-progress-${pots[0].name.toLowerCase().replace(/\s+/g, '-')}`
    );
    ...
    const potItem = screen.getByTestId(
      `pot-item-${pots[0].name.toLowerCase().replace(/\s+/g, '-')}`
    );
    expect(
      within(potItem).getByText(`Total of ${pots[0].target}`)
    ).toBeInTheDocument();

    // For the second pot, check the progress bar
    const secondPotProgressBar = screen.getByTestId(
      `pot-progress-${pots[1].name.toLowerCase().replace(/\s+/g, '-')}`
    );
    ...
    // Verify that there is text with the target description
    const secondPotItem = screen.getByTestId(
      `pot-item-${pots[1].name.toLowerCase().replace(/\s+/g, '-')}`
    );
    expect(
      within(secondPotItem).getByText(`Total of ${pots[1].target}`)
    ).toBeInTheDocument();
  });
  ...
  it('updates pot money correctly', () => {
    ...
    expect(
      screen.getByTestId('withdraw-money-modal-saving')
    ).toBeInTheDocument();
    ...
    const confirmWithdrawButton = screen.getByTestId(
      'confirm-withdraw-btn-saving'
    );
