...
import {sortMenu, userRecurringBills} from "../lib/lits";
...
const RecurringBills = () => {
    ...
    return (
        <main className="w-full bg-beige-100 py-6 px-4 md:px-10 md:py-8 xl:px-10 gap-8 xl:gap-8 flex flex-col" data-testid="recurring-bills-page">
            <h1 className="text-preset-1" aria-label="Recurring Bills">Recurring Bills</h1>
            <section className="flex flex-col gap-3 md:flex-wrap md:flex-row md:gap-6">
                <article className="xl:w-1/2 flex flex-col gap-5 w-full md:flex-row xl:flex-col">
                    <section
                        ...
                        role="contentinfo" aria-label="Total bills">
                    ...
                    <section className="flex flex-col bg-white p-5 rounded-lg gap-5 md:flex-1 xl:h-1/2 xl:flex-grow-0" role="region" aria-label="Summary">
                ...
                <section className="flex flex-col bg-white p-5 rounded-lg gap-5 w-full xl:flex-1" role="region" aria-label="Bills List">
                        ...
                        <Menu as="div" className="relative inline-block text-left">
                            <div className="flex items-center gap-3 text-preset-5 text-grey-500">
                                ...
                                >
                                    ...
                                    <ChevronDownIcon aria-hidden="true" className="-mr-1 size-5 text-gray-400 hidden md:flex" />
                    ...
                    <table>
                        ...
                        <tbody>
                            {userRecurringBills.map((item, index) => (
                                <tr className="border-b last:border-b-0" key={index} data-testid={`bill-item-${index}`}>
                                    <th className="py-5 flex flex-col gap-2 md:flex-row w-full md:flex md:justify-between">
                                        <div className="flex items-center gap-4 md:w-1/3">
                                            <img src={item.icon} className="rounded-full h-8" alt={`${item.name} icon`} />
                                            <h3 className="text-preset-4-bold capitalize">{item.name}</h3>
                                        </div>
                                        <div className="items-center gap-3 md:flex hidden">
                                            <span className="text-preset-4 capitalize text-green">{item.order}</span>
                                            <CheckCircleIcon className={item.status === "paid" ? "fill-green h-4" : "fill-red h-4"} />
                                        </div>
                                        <div className="flex items-center justify-between md:w-1/3 md:justify-end">
                                            ...
                                            <span className={item.status === "paid" ? "text-green text-preset-4-bold" : "text-grey-900 text-preset-4-bold text-center"}>
                                                {item.amount}
                                            </span>
                                        </div>
                                    </th>
