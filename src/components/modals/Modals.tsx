...
const Modals: React.FC<ModalsProps> = ({ addNewPot, addNewBudget }) => {
  ...
  const ModalBackdrop = () =>
    isModalOpen && <div className='fixed inset-0 bg-black/30 z-0'></div>;
  ...
  const ColorTagMenu = () => {
    const currentColorTag = colorTags.find(
    ...
    return (
      <Menu as='div' className='relative w-full'>
        <div className='flex flex-col w-full gap-1 text-preset-5-bold text-grey-500'>
          <p>Color Tag</p>
          <MenuButton className='group inline-flex py-[0.75rem] px-5 h-full capitalize items-center justify-between gap-x-1.5 rounded-md bg-white font-semibold text-gray-900 ring-1 ring-gray-300 hover:bg-gray-50 '>
            <div className='flex gap-2'>
              <div
                className='w-4 h-4 rounded-full'
                style={{ background: currentColorTag?.hex || '#000' }}
              ></div>
              <p>{currentColor}</p>
            </div>
            <ChevronDownIcon
        ...
        <MenuItems className='absolute w-full max-h-[250px]  z-10 mt-2 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black/5 scrollbar-thin scrollbar-thumb-rounded-full scrollbar-track-rounded-full scrollbar-thumb-grey-white scrollbar-track-gray-50  overflow-y-scroll'>
          <div className='py-1   '>
            {colorTags.map(color => (
              <MenuItem key={color.name}>
                <li
                  onClick={() => {
                    setPotColor(color.name);
          ...
          <DialogPanel className='w-full flex flex-col gap-5 max-w-md md:max-w-xl rounded-xl bg-white shadow-md p-6 md:p-8'>
            ...
            <div className='flex flex-col gap-4'>
              <Field className='flex flex-col gap-1 w-full'>
                ...
                <div className='group w-full flex py-2 px-5 items-center justify-between rounded-md ring-1 ring-gray-300 hover:bg-gray-50'>
                  <Input
                    type='text'
                ...
                <p className='text-preset-5 text-grey-300 self-end mt-1'>
